...
(define-struct world[t a c1now c1go slide-h slide-v drag?])



(define INITIAL-WORLD (make-world 0 0 300 300 900 650 false))
 ...
 (cond
   [(= 0 (remainder (world-t w) 5))
    (make-world
     ...
     (* (world-a w) 100)(world-slide-h w)(world-slide-v w)(world-drag? w))
    ]
   [else
    (make-world
     (add1 (world-t w))
     (abs (inexact->exact (rs-ith/left SONG (pstream-current-frame ps))))
     (/ (+ (world-c1now w) (world-c1go w)) 2)
     (world-c1go w)(world-slide-h w)(world-slide-v w)(world-drag? w))
    ]
 ))
...
(define (mouse-event w x y event)
 ...
 (cond
   [(mouse=? event "button-up")
    (make-world (world-t w) (world-a w) (world-c1now w) (world-c1go w) (world-slide-h w) (world-slide-v w) false)
    ]
   [(mouse=? event "drag")
    (cond
      [(boolean=? true (world-drag? w))
       (cond
         [(and (> x (- 900 250)) (< x (+ 900 250)))
          (make-world (world-t w) (world-a w) (world-c1now w) (world-c1go w) x (world-slide-v w) true)]
         [else 
          (make-world (world-t w) (world-a w) (world-c1now w) (world-c1go w) (world-slide-h w) (world-slide-v w) true)]
         )]
      [else 
       (cond
         [(and
           (and (> x (- (world-slide-h w) 10)) (< x (+ (world-slide-h w) 10)))
           (and (> y (- (world-slide-v w) 10)) (< y (+ (world-slide-v w) 10))))
          (make-world (world-t w) (world-a w) (world-c1now w) (world-c1go w)
                      x (world-slide-v w) true)]
         [else w])])
    ]
   [else w])
 ))
